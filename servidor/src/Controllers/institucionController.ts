...
class InstitucionController{
    ...
    public async getOneInstitucion (req: Request, res: Response): Promise<any>{
        const { id } = req.params;
        const unaInstitucion = await pool.query('SELECT * FROM institucion WHERE idInst = ?', [id]);
    ...
    public async modificarInstitucion (req: Request, res: Response): Promise<void>{
        const {id} = req.params;
        await pool.query('UPDATE institucion set ? WHERE idInst = ?', [req.body, id]);
