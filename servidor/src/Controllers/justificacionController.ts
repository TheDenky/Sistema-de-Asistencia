...
class JustificacionController{
    ...
    public async getOneJustificacion (req: Request, res: Response): Promise<any>{
        const { id } = req.params;
        const unaInstitucion = await pool.query('SELECT * FROM justificacion WHERE idJust = ?', [id]);
    ...
    public async modificarJustificacion (req: Request, res: Response): Promise<void>{
        const {id} = req.params;
        await pool.query('UPDATE justificacion set ? WHERE idJust = ?', [req.body, id]);
